...
func emitSchemaFields(b *strings.Builder, goName string, r ResourceDef) {
	...
	for _, field := range schema.SortedFields(r.Schema.Fields, pkColumn(r)) {
		...
		b.WriteString(FieldChainExpr(def))
		b.WriteString(",\n")
	}
	b.WriteString("}\n\n")
}

// FieldChainExpr renders a FieldDef as its schema builder chain, e.g.
// `schema.String().Required().Searchable()`. Codegen uses it for the
// SchemaFields var; dsl.EmitGo reuses it so converted Go DSL matches
// generated code byte-for-byte.
func FieldChainExpr(def *schema.FieldDef) string {
