...
func TestSymmetricEncryptDecryptStringKey(t *testing.T) {
	...
	// Bare-hex payload (no envelope) for string keys, like upstream rawEncrypt.
	for _, c := range a {
...
func TestSymmetricEnvelopeRoundTrip(t *testing.T) {
	...
	// Retired current version still decrypts via retained keys.
	old := cfg
	...
	// Legacy bare-hex payloads decrypt via the legacy secret.
	...
	// Unknown envelope versions and missing legacy secrets fail closed.
...
func TestEncryptFormatsDifferAndDecryptFallsBack(t *testing.T) {
	// The legacy $brick$ (AES-GCM) and upstream (XChaCha20) formats differ.
	...
	// DecryptStringCompatible spans rotation across both formats.
