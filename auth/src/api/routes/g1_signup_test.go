...
//   - Form path pins 422 on banana (f1_forms_test.go:134-143).
...
func TestG1_SignUpJSONMistypedRememberMeIsClientError(t *testing.T) {
...
func TestG1_SignUpJSONMistypedNameIsClientError(t *testing.T) {
	db := newParityMemAdapter()
	opts := emailAuthTestOptions(db)
	api := g1SignUpAPI(t, opts)
	resp := api.Post("/api/auth/sign-up/email", map[string]any{
...
func TestG1_SignUpJSONValidStillSucceeds(t *testing.T) {
	db := newParityMemAdapter()
	opts := emailAuthTestOptions(db)
	api := g1SignUpAPI(t, opts)
	resp := api.Post("/api/auth/sign-up/email", map[string]any{
...
func TestG1_SignUpUnmarshalMistypedKnownKeysError(t *testing.T) {
