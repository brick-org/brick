...
func TestClientIP_SignUpStoresIPAndUserAgent(t *testing.T) {
...
func TestClientIP_SignInStoresIPAndUserAgent(t *testing.T) {
	db := newParityMemAdapter()
	opts := emailAuthTestOptions(db)
	api := ipuaTestAPI(t, opts)
...
func TestClientIP_NoRequestStoresEmptyStrings(t *testing.T) {
	db := newParityMemAdapter()
	opts := emailAuthTestOptions(db)
	now := time.Now().UTC()
...
func TestClientIP_ClientIPResolution(t *testing.T) {
	cases := []struct {
		name      string
		xff       string
		xRealIP   string
