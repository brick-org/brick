...
func TestHookedParity_UpdateManyFiresUpdateHooks(t *testing.T) {
...
func TestHookedParity_UpdateManyBeforeAborts(t *testing.T) {
	ctx := context.Background()
	inner := newMemoryAdapter()
	if _, err := inner.Create(ctx, "user", map[string]any{"id": "1", "role": "member"}, nil); err != nil {
		t.Fatal(err)
...
func TestHookedParity_AfterHookErrorsAreLoggedAndPropagated(t *testing.T) {
	ctx := context.Background()
...
func TestHookedParity_TransactionPropagatesHooks(t *testing.T) {
	ctx := context.Background()
	inner := newMemoryAdapter()
...
func TestHookedParity_BeforeAbortSemantics(t *testing.T) {
	ctx := context.Background()
	inner := newMemoryAdapter()
