...
func TestF8_StatelessDefaultsJweCache(t *testing.T) {
	api := newTestAPI(t)
...
func TestF8_StatelessMaxAgeFollowsExpiresIn(t *testing.T) {
	api := newTestAPI(t)
	a, err := auth.BetterAuth(auth.Options{
		Secret:  f8Secret,
		Adapter: api.Adapter(),
...
func TestF8_StatefulStaysOptIn(t *testing.T) {
	api := newTestAPI(t)
	a, err := auth.BetterAuth(auth.Options{
		Secret:  f8Secret,
		Adapter: api.Adapter(),
...
func TestF8_SecondaryOnlyStaysOptInCacheButStoresAccount(t *testing.T) {
	api := newTestAPI(t)
	a, err := auth.BetterAuth(auth.Options{
		Secret:           f8Secret,
		Adapter:          api.Adapter(),
...
func TestF8_MissingBaseURLWarn(t *testing.T) {
	api := newTestAPI(t)
