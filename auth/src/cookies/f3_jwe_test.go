...
func TestF3_JWEHeaderExactlyThreeKeys(t *testing.T) {
...
func TestF3_JWECrossStackCookiesToCrypto(t *testing.T) {
	session, user := f3SessionUser()
	token, err := CreateSessionCacheJWE(f3Secret, session, user, "1", 5*time.Minute)
	if err != nil {
...
func TestF3_JWECrossStackCryptoToCookies(t *testing.T) {
	session, user := f3SessionUser()
