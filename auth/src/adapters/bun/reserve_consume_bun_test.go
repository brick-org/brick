...
func TestBunReserveConsume_ReserveRoundTrip(t *testing.T) {
	ctx := context.Background()
...
func TestBunReserveConsume_DuplicateIsTyped(t *testing.T) {
	ctx := context.Background()
	a := f10VerificationAdapter(t)
...
func TestBunReserveConsume_ConsumeOnce(t *testing.T) {
	ctx := context.Background()
	a := f10VerificationAdapter(t)
	now := time.Now().UTC().Truncate(time.Second)
